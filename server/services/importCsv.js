...
// your consumer keys (or just prefer hand-editing a spreadsheet). Writes
// into the exact same tables services/sync.js does via services/store.js,
// so the dashboard, player detail, and digest all work identically
// regardless of where the data came from. Match Prep still needs a real
// CHPP connection (it needs live fixture + opponent data), and this data
// gets superseded — not merged — once you actually connect, since a real
// Hattrick team has a different, real team ID.
const { parseCsv } = require('../lib/csv');
const { setSetting, withTransaction, db } = require('../db');
const { estimateValue } = require('./valuation');
const { csvRecordToPlayerInput, stablePlayerId } = require('./csvSchema');
 ...
 * @returns {{ok: true, playerCount, teamTsi, teamWorth} | {ok: false, errors: string[]}}
 */
function importSquadCsv(csvText, teamName) {
  ...
  const inputs = records.map((record, i) => {
    const { errors: rowErrors, input } = csvRecordToPlayerInput(record, i + 2); // +2: header is row 1, records are 1-indexed for humans
  ...
  const runInTransaction = () => {
    ...
    for (const p of resolved) {
      ...
      const row = {
        ...
        nickname: '', ageYears: p.ageYears, ageDays: null,
        positionCode: p.positionCode, specialty: p.specialtyId ? SPECIALTIES[p.specialtyId] : null,
        tsi: p.tsi, salary: p.salary, form: p.form, experience: null, leadership: null,
    ...
    store.upsertTeamSnapshot({
      ...
      cash: null, weeklyIncome: null, weeklyExpenses: null,
    });
  ...
  logImport('ok', `${result.playerCount} players, team TSI ${result.teamTsi}`);
